...
static bool uid_insert_cell(struct statistic *s, const void *sample) {
    const libtop_psamp_t *psamp = sample;
    char buf[GENERIC_INT_SIZE];

    if(-1 == snprintf(buf, sizeof(buf), "%" PRId32,
		      (int32_t)psamp->uid))
	return true; /*error*/
...
struct statistic *top_uid_create(WINDOW *parent, const char *name) {
    return create_statistic(STATISTIC_UID, parent, NULL, &callbacks, 
			    name);
}
